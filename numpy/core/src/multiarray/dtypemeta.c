...
string_unicode_common_instance(PyArray_Descr *descr1, PyArray_Descr *descr2)
{
    if (descr1->elsize >= descr2->elsize) {
        return ensure_dtype_canonical(descr1);
    }
    else {
        return ensure_dtype_canonical(descr2);
    }
}
...
void_ensure_canonical(PyArray_Descr *self)
{
    if (self->subarray != NULL) {
        PyArray_Descr *new_base = ensure_dtype_canonical(self->subarray->base);
        if (new_base == NULL) {
            return NULL;
        }
        ...
        for (Py_ssize_t i = 0; i < field_num; i++) {
            ...
            PyArray_Descr *field_descr = ensure_dtype_canonical(
...
dtypemeta_wrap_legacy_descriptor(PyArray_Descr *descr)
{
    ...
    dtype_class->ensure_canonical = ensure_native_byteorder;
    ...
    if (PyTypeNum_ISUSERDEF(descr->type_num)) {
        ...
        if (descr->type_num == NPY_VOID) {
            ...
            dtype_class->ensure_canonical = void_ensure_canonical;
        }
        else {
