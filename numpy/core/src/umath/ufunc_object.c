...
ufunc_resolve_ufunc_impl(
        PyUFuncObject *ufunc, PyArray_DTypeMeta *dtypes[NPY_MAXARGS],
        PyUFuncImplObject **ufunc_impl)
{
    *ufunc_impl = NULL;
    ...
    PyObject *dtype_tuple = PyTuple_New(nop);
    PyObject *cached;
    if (dtype_tuple == NULL) {
        return -1;
    }
    ...
    for (Py_ssize_t i = 0; i < nop; i++) {
        ...
        if (dtypes[i] == NULL ) {
            ...
            is_any_dtype_abstract |= dtypes[i]->abstract;
        }
        Py_INCREF(tmp);
        PyTuple_SET_ITEM(dtype_tuple, i, tmp);
    }
        ...
        NPY_CASTING casting)
{
     ...
     * existing system.
     */
    // TODO: This _will_ break numba since they change UFuncs loops in a wonky
    //       way. I feel this should be OK, let them fix it on their end.
    ...
    for (int i = 0; i < ufunc->nargs; i++) {
        dtypes[i] = (PyArray_DTypeMeta *)out_descriptors[i];
        Py_INCREF(dtypes[i]);
    }
